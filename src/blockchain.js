...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            let message = address + ":" + currentTime.toString()+":starRegistry";
            resolve(message);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let currentTime = parseInt(new Date().getTime().toString().slice(0, -3));
            let timePassed = currentTime-msgTime;
            if (timePassed>300) {
                reject("timeout");
            } else {
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async (blk)=>{
                if (await blk.validate()) {
                    if (blk.height != 0) {
                        let previousHash = self.chain[blk.height-1].hash;
                        if (blk.previousBlockHash !== previousHash) {
                            errorLog.push(new Error(blk + " not linked correctly"));
                        }
                    }
                } else {
                    errorLog.push(new Error(blk + " cannot be validated"))
                }
            })
            resolve(errorLog);
        });
